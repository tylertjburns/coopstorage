...
from coopstorage.storage.loc_load.data.mongo import mongo_connection_args
...
MONGO_PROD_LOC_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=LOCATIONS,
    connection_args=mongo_connection_args(), facade=LOCATION_FACADE,
)
MONGO_PROD_LOAD_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=LOADS,
    connection_args=mongo_connection_args(), facade=LOAD_FACADE,
)
MONGO_PROD_TREQ_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=TRANSFER_REQUESTS,
    connection_args=mongo_connection_args(), facade=TRANSFER_REQUEST_FACADE,
)
MONGO_TEST_LOC_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=f"{LOCATIONS}_TEST",
    connection_args=mongo_connection_args(), facade=LOCATION_FACADE,
)
MONGO_TEST_LOAD_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=f"{LOADS}_TEST",
    connection_args=mongo_connection_args(), facade=LOAD_FACADE,
)
MONGO_TEST_TREQ_DATA = MongoCollectionDataStore(
    db_name=DB_NAME, collection_name=f"{TRANSFER_REQUESTS}_TEST",
    connection_args=mongo_connection_args(), facade=TRANSFER_REQUEST_FACADE,
)
...
if __name__ == "__main__":
    ...
    def t01():
        ...
        results = MONGO_TEST_LOC_DATA.add([loc]).get()
        ret = [Location.from_jsonable_dict(data) for _, data in results.items()]
        pprint(ret)
    ...
    def t02():
        ...
        results = MONGO_TEST_LOC_DATA.add([loc]).get(ids=[loc.Id])
        loc = Location.from_jsonable_dict(results[loc.Id])
        loc.store_loads(['l1', 'l2'])

        results = MONGO_TEST_LOC_DATA.update([loc]).get(ids=[loc.Id])
        loc = Location.from_jsonable_dict(results[loc.Id])
        pprint(loc)
