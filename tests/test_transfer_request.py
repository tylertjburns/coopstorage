...
- TransferRequest.Ready — load accessible, dest has room
...
class TestTransferRequestCriteria(unittest.TestCase):
    ...
    def test_new_load_only(self):
        """Criteria with only new_load is valid."""
        c = TransferRequestCriteria(new_load=_load())
        self.assertIsNotNone(c.new_load)
    ...
    def test_query_args_only(self):
        ...
        self.assertIsNotNone(c.load_query_args)
...
class TestTransferRequestVerify(unittest.TestCase):
    ...
    def test_type1_store_new_load_at_dest(self):
        """source=None, load firm, dest firm → store new load."""
        tr = TransferRequest(
            criteria=TransferRequestCriteria(new_load=_load()),
            load=_load(),
            source_loc=None,
            dest_loc=_loc(),
        )
    ...
    def test_type2_transfer_from_source_to_dest(self):
        """source firm, load firm, dest firm → transfer."""
        tr = TransferRequest(
            criteria=TransferRequestCriteria(new_load=_load()),
            load=_load(),
            source_loc=_loc('SRC'),
    ...
    def test_type3_remove_from_source(self):
        """source firm, load firm, dest=None → remove."""
        tr = TransferRequest(
            criteria=TransferRequestCriteria(new_load=_load()),
            load=_load(),
            source_loc=_loc('SRC'),
...
class TestTransferRequestReady(unittest.TestCase):
    ...
    def test_ready_new_load_to_empty_dest(self):
    ...
    def test_ready_transfer_accessible_load(self):
    ...
    def test_not_ready_load_not_accessible_in_source(self):
        """L1 is buried in a FIFO loc behind L2 — not accessible."""
        src = _fifo_loc('SRC', capacity=3)
        src.store_loads(['L1', 'L2'])  # L1 first in, but L2 came after
    ...
    def test_ready_remove_only(self):
        """Remove from source, no dest — always ready if load accessible."""
        src = _loc('SRC', capacity=3)
        src.store_loads(['L1'])
        tr = TransferRequest(
            criteria=TransferRequestCriteria(new_load=_load()),
...
class TestTransferRequestSerialization(unittest.TestCase):
    ...
    def test_round_trip_with_both_locs(self):
        ...
        self.assertEqual(restored.load.id, 'L99')
